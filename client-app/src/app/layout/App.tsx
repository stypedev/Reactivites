  ...
  function handleCreateOrEditActivity(activity: Activity) {
    ...
    if(activity.id){
      agent.Activities.update(activity).then(() => {
        setActivities([...activities.filter(x => x.id !== activity.id),activity]);// imamo activity.id pa se radi o editiranju ... p.s. filtrirj sve različite od našega activity.id-ja!
      ...
      agent.Activities.create(activity).then(() => {
        setActivities([...activities,activity]); //nemamo id, pa dodaj novu , dakle radi se o createActivity, p.s. napravi spread activity.ja kojeg prosljeđujemo i generiraj uuid za novi activity  
        setSelectedActivity(activity);
        setEditMode(false);
        setSubmitting(false);
      })
